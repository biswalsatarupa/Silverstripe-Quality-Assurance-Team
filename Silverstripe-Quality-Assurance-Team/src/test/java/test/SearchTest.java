...
public class SearchTest extends BaseTest {
	...
	@Test(dataProvider = "getTestData", priority = 3,enabled = true,description="To verify the keyword search with Special characters in Silvertstripe Application")
	public void verifyKeywordSearchWithSpecialCharactersInSilverstripe(ITestContext context,String UserName, String ExpectedResult) {
		driver = new DriverUtils().getDriver();
		searchPageObj.SearchTextWithSpecialCharacters(url, UserName, ExpectedResult);
	}
	
	@Test(dataProvider = "getTestData", priority = 4, enabled = true,description= "To verify the keyword search with combination of special characters and text in Silverstripe application")
	public void verifyKeywordSearchWithCombinationOfSpeacialCharactersAndText(ITestContext context, String UserName, String SearchedText){
	   driver = new DriverUtils().getDriver();
	   searchPageObj.SearchWithCombinationOfSpecialCharactersAndText(url, UserName, SearchedText);
	}
